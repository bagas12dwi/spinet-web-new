...
@section('content')
    <div class="container-xl">
        <a href="{{ route('admin.kit.index') }}" class="btn btn-primary text-white mb-3">
        ...
        <form action="{{ route('admin.kit.update', $media->id) }}" method="post" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="mb-3">
                ...
                <input type="text" class="form-control" name="title" id="title" aria-describedby="helpTitle"
                    placeholder="Masukkan Nama Media" value="{{ $media->title }}" />
                <small id="helpTitle" class="form-text text-muted">Masukkan nama media</small>
            </div>
            <div class="mb-3">
                <label for="description" class="form-label">Deskripsi</label>
                <textarea class="form-control" name="description" id="description" rows="3">{{ $media->description }}</textarea>
            </div>
            <img src="{{ URL::asset('storage/' . $media->thumbnails) }}" style="width: 10em" alt="">
            <div class="mb-3">
            ...
            <input type="hidden" name="oldThumbnails" value="{{ $media->thumbnails }}">
            <input type="hidden" name="oldDocument" value="{{ $media->document_path }}">
