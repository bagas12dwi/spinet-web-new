...
@section('content')
    <div class="container-xl">
            ...
            <div class="col-auto">
                <div class="page-utilities">
                    <div class="row g-2 justify-content-start justify-content-md-end align-items-center">
                        <div class="col-auto">
                            <a class="btn app-btn-primary" href="{{ route('admin.diskusi.create') }}">
        ...
        <table class="table">
            ...
            <tbody>
                @forelse ($diskusi as $key => $item)
                    <tr>
                        <th scope="row">{{ $key + 1 }}</th>
                        <td>{{ $item->title }}</td>
                        <td>
                            <div class="d-flex gap-1">
                                <a href="{{ route('admin.diskusi.edit', $item->id) }}" class="btn btn-warning">
                                ...
                                <form id="deleteForm" action="{{ route('admin.diskusi.destroy', $item->id) }}"
                                    method="post">
                                    @csrf
                                    @method('DELETE')
        ...
        <nav class="app-pagination mt-5">
            <ul class="pagination justify-content-center">
                {{-- Previous Page Link --}}
                @if ($diskusi->onFirstPage())
                    <li class="page-item disabled" aria-disabled="true">
                        <a class="page-link" href="#" tabindex="-1" aria-disabled="true">Sebelumnya</a>
                    </li>
                @else
                    <li class="page-item">
                        <a class="page-link" href="{{ $diskusi->previousPageUrl() }}" rel="prev">Sebelumnya</a>
                    </li>
                @endif

                {{-- Pagination Elements --}}
                @foreach ($diskusi->links()->elements as $element)
                ...
                {{-- Next Page Link --}}
                @if ($diskusi->hasMorePages())
                    <li class="page-item">
                        <a class="page-link" href="{{ $diskusi->nextPageUrl() }}" rel="next">Berikutnya</a>
                    </li>
                @else
                    <li class="page-item disabled" aria-disabled="true">
