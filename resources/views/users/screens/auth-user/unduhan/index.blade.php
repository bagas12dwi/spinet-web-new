...
@section('konten')
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h4 class="fw-bold">Media yang diunduh</h4>
    </div>
    ...
    @if ($download->count() > 1)
        <nav class="app-pagination my-5">
            <ul class="pagination justify-content-center">
                {{-- Previous Page Link --}}
                @if ($download->onFirstPage())
                    <li class="page-item disabled" aria-disabled="true">
                        <a class="page-link" href="#" tabindex="-1" aria-disabled="true">Sebelumnya</a>
                    </li>
                @else
                    <li class="page-item">
                        <a class="page-link" href="{{ $download->previousPageUrl() }}" rel="prev">Sebelumnya</a>
                    </li>
                @endif

                {{-- Pagination Elements --}}
                @foreach ($download->links()->elements as $element)
                ...
                {{-- Next Page Link --}}
                @if ($download->hasMorePages())
                    <li class="page-item">
                        <a class="page-link" href="{{ $download->nextPageUrl() }}" rel="next">Berikutnya</a>
                    </li>
                @else
                    <li class="page-item disabled" aria-disabled="true">
